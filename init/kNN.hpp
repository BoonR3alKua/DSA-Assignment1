...
public:
    ...
    Dataset extract(int startRow = 0, int endRow = -1, int startCol = 0, int endCol = -1) const
    {
        ...
        if(endRow < -1 || endC < -1) throw std::out_of_range("get(): Out of range");
        if((endRow > -1 && startRow > endRow) || (endCol > -1 && startCol > endCol)) throw std::out_of_range("get(): Out of range");
    ...
    Dataset predict(const Dataset& X_train, const Dataset& Y_train, const int k) const
    {
        ...
        for (int i = 0; i < this->data->length(); i++)
        {
            ...
            int predictedLabel = findkNNLabel(distances, label, lengthTrain, k);
